...
local TENT_STASH_SLOTS = 10
local TENT_STASH_WEIGHT = 10000
            ...
            if PlayerData and PlayerData.items then
                for _, item in pairs(PlayerData.items) do
                    if item and item.name then
                        inventory[item.name] = (inventory[item.name] or 0) + (item.amount or 0)
                    end
                end
            end
        end
        ...
        if items then
            for itemName, itemData in pairs(items) do
                if type(itemData) == 'table' then
                    inventory[itemName] = itemData.amount or itemData.count or 0
                else
...
local function openTentStash(stashId)
    ...
    elseif Inventory == 'qb' or Inventory == 'qs' then
        local other = {
            maxweight = TENT_STASH_WEIGHT,
            slots = TENT_STASH_SLOTS,
        }
...
function FuelSystem:addFuel(itemtype, inputAmount, duration)
    ...
    local availableCount = getInventoryItemCount(itemtype)
...
-- Item usage handler
local function handleInventoryItemUse(name, slotId, metadata)
...
AddEventHandler('ox_inventory:usedItem', function(name, slotId, metadata)
    handleInventoryItemUse(name, slotId, metadata)
end)

RegisterNetEvent('camping:client:qsUsedItem', function(itemData)
    if Inventory ~= 'qs' or not itemData then return end
    handleInventoryItemUse(itemData.name, itemData.slot, itemData.metadata)
end)
...
RegisterNetEvent('campfire_cooking', function(recipe)
    ...
    local playerAmount = getInventoryItemCount(ingredient)
