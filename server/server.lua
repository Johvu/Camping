...
if Inventory == 'qs' then
    local function registerQSUsable(itemName)
        exports['qs-inventory']:CreateUsableItem(itemName, function(source, item)
            TriggerClientEvent('camping:client:qsUsedItem', source, {
                name = item and item.name or itemName,
    ...
    registerQSUsable(Config.tentItem)
    registerQSUsable(Config.campfireItem)
end
...
AddEventHandler('camping:saveCampingData', function(type, model, x, y, z, stashID, heading)
    ...
    if type == 'tent' and safeStashID ~= '' then
        ...
        elseif Inventory == 'qs' then
            exports['qs-inventory']:RegisterStash(src, safeStashID, 10, 10000)
        end
    end
end)
...
RegisterNetEvent('camping:AI', function(itemName, amount, meta)
    ...
    elseif Inventory == 'qs' then
        exports['qs-inventory']:AddItem(src, itemName, amount, nil, meta or nil)
    end
end)
...
RegisterNetEvent('camping:RI', function(itemName, amount, meta, slot)
    ...
    elseif Inventory == 'qs' then
        exports['qs-inventory']:RemoveItem(src, itemName, amount, slot or nil, meta or nil)
    end
...
RegisterNetEvent('camping:server:spawnTent', function(x, y, z, h, randomModel, stashId, slot)
    ...
    elseif Inventory == 'qs' then
        exports['qs-inventory']:RemoveItem(src, Config.tentItem, 1, slot or nil, nil)
    end
...
RegisterNetEvent('camping:server:spawnCampfire', function(x, y, z, h, fireModel, campfireId, slot)
    ...
    elseif Inventory == 'qs' then
        exports['qs-inventory']:RemoveItem(src, Config.campfireItem, 1, slot or nil, nil)
    end
...
RegisterNetEvent('camping:createTentStash', function(stashId)
    ...
    elseif Inventory == 'qb' then
        -- add your qb stash function
    elseif Inventory == 'qs' then
        exports['qs-inventory']:RegisterStash(src, stashId, 10, 10000)
    end
end)
